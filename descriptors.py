...
def cluster_features(img_descs, test_image, cluster_model):
    """
    ...
    training_descs = img_descs #[the number of training images, keypoints, descriptor]
    all_train_descriptors = [desc for desc_list in training_descs for desc in desc_list] #[keypoints of all images , 128]
    ...
    # cluster center is the code book.
    ...
    ##################test image####################
    imgtest_set = dataset.Dataset("dataset_real2").get_test_set()
    y1 = []
    x1 = None
    img_descs1 = []

    for class_number in range(len(imgtest_set)):
        img_paths = imgtest_set[class_number]
        ...
        for i in range(len(img_paths)):
            ...
            des1, y1 = sift(img, img_descs1, y1, class_number)
            """ des : descriptor
                y : class_number
            """
    desTest = []
    sift_test = cv2.xfeatures2d.SIFT_create()
    kp, des_test = sift_test.detectAndCompute(test_image, None)
    desTest.append(des_test)

    img_clustered_words_test = [cluster_model.predict(raw_words) for raw_words in des1]
    #print(np.shape(img_clustered_words_test))
    #print(np.shape(img_clustered_words_test[0]))
    #print(img_clustered_words_test[0][:])

    imgtest_bow_hist = np.array(
        [np.bincount(clustered_words, minlength=n_clusters) for clustered_words in img_clustered_words_test])
    X_test = imgtest_bow_hist
    print(X_test[0][:])
    print('done predicting BoW histograms.')

    testimg_clustered_word = [cluster_model.predict(rawwords) for rawwords in desTest]
    testimg_bow_hist = np.array([np.bincount(clusteredWords, minlength=n_clusters) for clusteredWords in testimg_clustered_word])

    return X, X_test, y1, testimg_bow_hist, cluster_model

def cluster_test(img_descs, cluster_model):
    """
        Cluster the training features using the cluster_model
        and convert each set of descriptors in img_descs
        to a Visual Bag of Words histogram.
    ...
    all_train_descriptors = [desc for desc_list in training_descs for desc in
                             desc_list]  # [keypoints of all images , 128]
    all_train_descriptors = np.array(all_train_descriptors)
    ...
    # compute set of cluster-reduced words for each image
    # cluster center is the code book.
    img_clustered_words = [cluster_model.predict(raw_words) for raw_words in img_descs] #clustered keypoints in each image, [all images, keypoints]

    # finally make a histogram of clustered word counts for each image. These are the final features.
    # 각 이미지의 분류된 키포인트가 k개의 기준 중 어디에 얼마나 분포하는지에 대한 히스토그램.
    img_bow_hist = np.array(
        [np.bincount(clustered_words, minlength=n_clusters) for clustered_words in img_clustered_words])
